"""
NSE Finnifty Option Chain Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('finnifty_option_chain_collector.log', encoding='utf-8'),
...
# Configuration
EXPIRY_API_URL = "https://www.nseindia.com/api/option-chain-contract-info?symbol=FINNIFTY"
...
MONGO_COLLECTION_NAME = os.getenv('MONGO_FINNIFTY_OPTION_CHAIN_COLLECTION_NAME', 'finnifty_option_chain_data')
MONGO_USERNAME = os.getenv('MONGO_USERNAME', None)
MONGO_PASSWORD = os.getenv('MONGO_PASSWORD', None)


class NSEFinniftyOptionChainCollector:
    """Collects and stores NSE Finnifty Option Chain data in MongoDB"""
    ...
    def _fetch_expiry_dates_with_retry(self) -> Optional[str]:
        ...
        if cached_expiry:
            logger.info(f"Using cached FINNIFTY expiry date: {cached_expiry}")
            return cached_expiry
        
        # If not in cache, fetch from API
        logger.info("FINNIFTY expiry date not found in cache, fetching from API...")
        headers = self._get_headers()
        
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                logger.info(f"Fetching FINNIFTY expiry dates from API (Attempt {attempt}/{MAX_RETRIES})")
                ...
                logger.info(f"Successfully fetched FINNIFTY expiry dates. Using first expiry: {first_expiry}")
                
                # Cache the expiry date for today
                try:
                    self.expiry_cache.set_expiry(SYMBOL, first_expiry)
                    logger.info(f"Cached FINNIFTY expiry date: {first_expiry}")
                ...
                if attempt < MAX_RETRIES:
                    ...
                    logger.error("All retry attempts failed for FINNIFTY expiry dates")
    ...
    def _fetch_option_chain_with_retry(self, expiry_date: str) -> Optional[Dict]:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                logger.info(f"Fetching FINNIFTY option chain data for expiry {expiry_date} (Attempt {attempt}/{MAX_RETRIES})")
                ...
                if not timestamp:
                    logger.error("Timestamp not found in FINNIFTY option chain response")
                    if attempt < MAX_RETRIES:
                        logger.info(f"Retrying in {RETRY_DELAY} seconds...")
                        time.sleep(RETRY_DELAY)
                        continue
                ...
                logger.info(f"Successfully fetched FINNIFTY option chain data. Timestamp: {timestamp}")
                return data
                ...
                if attempt < MAX_RETRIES:
                    ...
                    logger.error("All retry attempts failed for FINNIFTY option chain data")
    ...
    def _save_to_mongo(self, data: Dict) -> bool:
        ...
        try:
            ...
            if result.upserted_id:
                logger.info(f"Inserted new FINNIFTY record with timestamp: {timestamp}")
                return True
            elif result.modified_count > 0:
                logger.info(f"Updated existing FINNIFTY record with timestamp: {timestamp}")
                return True
            else:
                logger.debug(f"FINNIFTY record with timestamp {timestamp} already exists (no changes)")
                return True  # Still considered successful if no duplicates
                
        except pymongo.errors.DuplicateKeyError:
            logger.warning(f"Duplicate FINNIFTY record skipped for timestamp: {timestamp}")
            return True  # Duplicate prevention working correctly
            
        except Exception as e:
            logger.error(f"Failed to save FINNIFTY data to MongoDB: {str(e)}")
            return False
    ...
    def collect_and_save(self) -> bool:
        ...
        """
        try:
            logger.info("Starting NSE FINNIFTY option chain data collection...")
            
            # Step 1: Fetch expiry dates and pick the first one
            expiry_date = self._fetch_expiry_dates_with_retry()
            
            if not expiry_date:
                logger.error("Failed to fetch FINNIFTY expiry dates after all retries")
                return False
            ...
            if option_chain_data is None:
                logger.error("Failed to fetch FINNIFTY option chain data after all retries")
                return False
            ...
            if success:
                logger.info("NSE FINNIFTY option chain data collection completed successfully")
            else:
                logger.error("Failed to save FINNIFTY option chain data to MongoDB")
