"""
NSE MidcapNifty Option Chain Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('midcpnifty_option_chain_collector.log', encoding='utf-8'),
...
# Configuration
EXPIRY_API_URL = "https://www.nseindia.com/api/option-chain-contract-info?symbol=MIDCPNIFTY"
...
MONGO_COLLECTION_NAME = os.getenv('MONGO_MIDCPNIFTY_OPTION_CHAIN_COLLECTION_NAME', 'midcpnifty_option_chain_data')
MONGO_USERNAME = os.getenv('MONGO_USERNAME', None)
MONGO_PASSWORD = os.getenv('MONGO_PASSWORD', None)


class NSEMidcapNiftyOptionChainCollector:
    """Collects and stores NSE MidcapNifty Option Chain data in MongoDB"""
    ...
    def _fetch_expiry_dates_with_retry(self) -> Optional[str]:
        ...
        if cached_expiry:
            logger.info(f"Using cached MIDCPNIFTY expiry date: {cached_expiry}")
            return cached_expiry
        
        # If not in cache, fetch from API
        logger.info("MIDCPNIFTY expiry date not found in cache, fetching from API...")
        headers = self._get_headers()
        
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                logger.info(f"Fetching MIDCPNIFTY expiry dates from API (Attempt {attempt}/{MAX_RETRIES})")
                ...
                logger.info(f"Successfully fetched MIDCPNIFTY expiry dates. Using first expiry: {first_expiry}")
                
                # Cache the expiry date for today
                try:
                    self.expiry_cache.set_expiry(SYMBOL, first_expiry)
                    logger.info(f"Cached MIDCPNIFTY expiry date: {first_expiry}")
                ...
                if attempt < MAX_RETRIES:
                    ...
                    logger.error("All retry attempts failed for MIDCPNIFTY expiry dates")
    ...
    def _fetch_option_chain_with_retry(self, expiry_date: str) -> Optional[Dict]:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                logger.info(f"Fetching MIDCPNIFTY option chain data for expiry {expiry_date} (Attempt {attempt}/{MAX_RETRIES})")
                ...
                if not timestamp:
                    logger.error("Timestamp not found in MIDCPNIFTY option chain response")
                    if attempt < MAX_RETRIES:
                        logger.info(f"Retrying in {RETRY_DELAY} seconds...")
                        time.sleep(RETRY_DELAY)
                        continue
                ...
                logger.info(f"Successfully fetched MIDCPNIFTY option chain data. Timestamp: {timestamp}")
                return data
                ...
                if attempt < MAX_RETRIES:
                    ...
                    logger.error("All retry attempts failed for MIDCPNIFTY option chain data")
    ...
    def _save_to_mongo(self, data: Dict) -> bool:
        ...
        try:
            ...
            if result.upserted_id:
                logger.info(f"Inserted new MIDCPNIFTY record with timestamp: {timestamp}")
                return True
            elif result.modified_count > 0:
                logger.info(f"Updated existing MIDCPNIFTY record with timestamp: {timestamp}")
                return True
            else:
                logger.debug(f"MIDCPNIFTY record with timestamp {timestamp} already exists (no changes)")
                return True  # Still considered successful if no duplicates
                
        except pymongo.errors.DuplicateKeyError:
            logger.warning(f"Duplicate MIDCPNIFTY record skipped for timestamp: {timestamp}")
            return True  # Duplicate prevention working correctly
            
        except Exception as e:
            logger.error(f"Failed to save MIDCPNIFTY data to MongoDB: {str(e)}")
            return False
    ...
    def collect_and_save(self) -> bool:
        ...
        """
        try:
            logger.info("Starting NSE MIDCPNIFTY option chain data collection...")
            
            # Step 1: Fetch expiry dates and pick the first one
            expiry_date = self._fetch_expiry_dates_with_retry()
            
            if not expiry_date:
                logger.error("Failed to fetch MIDCPNIFTY expiry dates after all retries")
                return False
            ...
            if option_chain_data is None:
                logger.error("Failed to fetch MIDCPNIFTY option chain data after all retries")
                return False
            ...
            if success:
                logger.info("NSE MIDCPNIFTY option chain data collection completed successfully")
            else:
                logger.error("Failed to save MIDCPNIFTY option chain data to MongoDB")
