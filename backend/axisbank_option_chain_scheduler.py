"""
Cronjob Scheduler for NSE Axis Bank Option Chain Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('axisbank_option_chain_scheduler.log', encoding='utf-8'),
...
STATUS_FILE = 'axisbank_option_chain_scheduler_status.json'
...
def run_collector():
    """Execute the NSE Axis Bank option chain data collector"""
    collector = None
    try:
        now = datetime.now()
        ...
        logger.info("=" * 60)
        logger.info(f"Axis Bank Option Chain Cronjob triggered at {now}")
        logger.info("=" * 60)
        ...
        if success:
            logger.info("Axis Bank Option Chain Cronjob completed successfully")
        else:
            logger.error("Axis Bank Option Chain Cronjob completed with errors")
        ...
        logger.error(f"Axis Bank Option Chain Cronjob failed with error: {str(e)}")
...
def main():
    ...
    logger.info("Starting NSE Axis Bank Option Chain Data Collector Scheduler")
