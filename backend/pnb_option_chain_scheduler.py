"""
Cronjob Scheduler for NSE PNB Option Chain Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('pnb_option_chain_scheduler.log', encoding='utf-8'),
...
STATUS_FILE = 'pnb_option_chain_scheduler_status.json'
...
def run_collector():
    """Execute the NSE PNB option chain data collector"""
    collector = None
    try:
        now = datetime.now()
        ...
        logger.info("=" * 60)
        logger.info(f"PNB Option Chain Cronjob triggered at {now}")
        logger.info("=" * 60)
        ...
        if success:
            logger.info("PNB Option Chain Cronjob completed successfully")
        else:
            logger.error("PNB Option Chain Cronjob completed with errors")
        ...
        logger.error(f"PNB Option Chain Cronjob failed with error: {str(e)}")
...
def main():
    ...
    logger.info("Starting NSE PNB Option Chain Data Collector Scheduler")
