"""
Cronjob Scheduler for NSE BankNifty Option Chain Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('banknifty_option_chain_scheduler.log', encoding='utf-8'),
...
def run_collector():
    """Execute the NSE BankNifty option chain data collector"""
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info(f"BankNifty Option Chain Cronjob triggered at {now}")
        logger.info("=" * 60)
        ...
        if success:
            logger.info("BankNifty Option Chain Cronjob completed successfully")
        else:
            logger.error("BankNifty Option Chain Cronjob completed with errors")
        ...
        logger.error(f"BankNifty Option Chain Cronjob failed with error: {str(e)}")
