"""
Cronjob Scheduler for NSE SBIN Option Chain Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('sbin_option_chain_scheduler.log', encoding='utf-8'),
...
STATUS_FILE = 'sbin_option_chain_scheduler_status.json'
...
def run_collector():
    """Execute the NSE SBIN option chain data collector"""
    collector = None
    try:
        now = datetime.now()
        ...
        logger.info("=" * 60)
        logger.info(f"SBIN Option Chain Cronjob triggered at {now}")
        logger.info("=" * 60)
        ...
        if success:
            logger.info("SBIN Option Chain Cronjob completed successfully")
        else:
            logger.error("SBIN Option Chain Cronjob completed with errors")
        ...
        logger.error(f"SBIN Option Chain Cronjob failed with error: {str(e)}")
...
def main():
    ...
    logger.info("Starting NSE SBIN Option Chain Data Collector Scheduler")
