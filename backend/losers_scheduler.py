...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('losers_scheduler.log', encoding='utf-8'),
...
def run_collector():
    """Execute the NSE Top 20 Losers data collector"""
    collector = None
    try:
        now = datetime.now()
        ...
        logger.info("=" * 60)
        logger.info(f"Top 20 Losers Cronjob triggered at {now}")
        logger.info("=" * 60)
        ...
        if success:
            logger.info("Top 20 Losers Cronjob completed successfully")
        else:
        ...
        logger.error(f"Top 20 Losers Cronjob failed with error: {str(e)}")
