"""
NSE Top 20 Gainers Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gainers_collector.log', encoding='utf-8'),
...
MONGO_COLLECTION_NAME = os.getenv('MONGO_GAINERS_COLLECTION_NAME', 'gainers_data')
MONGO_USERNAME = os.getenv('MONGO_USERNAME', None)
MONGO_PASSWORD = os.getenv('MONGO_PASSWORD', None)


class NSEGainersCollector:
    """Collects and stores NSE Top 20 Gainers data in MongoDB"""
    ...
    def _fetch_gainers_data_with_retry(self) -> Optional[Dict]:
        """
        Fetch Top 20 Gainers data from NSE API with retry logic
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                logger.info(f"Fetching Top 20 Gainers data (Attempt {attempt}/{MAX_RETRIES})")
                
                response = requests.get(GAINERS_API_URL, headers=headers, timeout=30)
                ...
                if not timestamp:
                    logger.error("Timestamp not found in gainers response")
                    if attempt < MAX_RETRIES:
                        logger.info(f"Retrying in {RETRY_DELAY} seconds...")
                        time.sleep(RETRY_DELAY)
                        continue
                ...
                logger.info(f"Successfully fetched Top 20 Gainers data. Timestamp: {timestamp}")
                ...
                if attempt < MAX_RETRIES:
                    ...
                    logger.error("All retry attempts failed for gainers data")
    ...
    def _save_to_mongo(self, data: Dict) -> bool:
        """
        Save entire gainers response to MongoDB
        ...
        try:
            ...
            if result.upserted_id:
                logger.info(f"Inserted new gainers record with timestamp: {timestamp}")
                return True
            elif result.modified_count > 0:
                logger.info(f"Updated existing gainers record with timestamp: {timestamp}")
                return True
            else:
                logger.debug(f"Gainers record with timestamp {timestamp} already exists (no changes)")
                return True  # Still considered successful if no duplicates
                
        except pymongo.errors.DuplicateKeyError:
            logger.warning(f"Duplicate gainers record skipped for timestamp: {timestamp}")
            return True  # Duplicate prevention working correctly
            
        except Exception as e:
            logger.error(f"Failed to save gainers data to MongoDB: {str(e)}")
            return False
    
    def collect_and_save(self) -> bool:
        """
        Main method to collect gainers data and save to MongoDB
        Returns: True if successful, False otherwise
        """
        try:
            logger.info("Starting NSE Top 20 Gainers data collection...")
            
            # Fetch gainers data
            gainers_data = self._fetch_gainers_data_with_retry()
            
            if gainers_data is None:
                logger.error("Failed to fetch gainers data after all retries")
                return False
            
            # Save entire response to MongoDB
            success = self._save_to_mongo(gainers_data)
            
            if success:
                logger.info("NSE Top 20 Gainers data collection completed successfully")
            else:
                logger.error("Failed to save gainers data to MongoDB")
