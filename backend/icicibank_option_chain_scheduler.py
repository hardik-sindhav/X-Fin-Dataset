"""
Cronjob Scheduler for NSE ICICI Bank Option Chain Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('icicibank_option_chain_scheduler.log', encoding='utf-8'),
...
STATUS_FILE = 'icicibank_option_chain_scheduler_status.json'
...
def run_collector():
    """Execute the NSE ICICI Bank option chain data collector"""
    collector = None
    try:
        now = datetime.now()
        ...
        logger.info("=" * 60)
        logger.info(f"ICICI Bank Option Chain Cronjob triggered at {now}")
        logger.info("=" * 60)
        ...
        if success:
            logger.info("ICICI Bank Option Chain Cronjob completed successfully")
        else:
            logger.error("ICICI Bank Option Chain Cronjob completed with errors")
        ...
        logger.error(f"ICICI Bank Option Chain Cronjob failed with error: {str(e)}")
...
def main():
    ...
    logger.info("Starting NSE ICICI Bank Option Chain Data Collector Scheduler")
