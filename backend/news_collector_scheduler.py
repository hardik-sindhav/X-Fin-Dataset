"""
Cronjob Scheduler for NSE News Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('news_collector_scheduler.log', encoding='utf-8'),
...
def get_scheduler_config():
    ...
    config = get_config_for_scheduler("news")
    if config:
        return config
...
def run_collector():
    ...
    # Check minimum interval - use IST for time comparisons
    now_ist = get_ist_now()
    config = get_scheduler_config()
    ...
    try:
        ...
        # Check if it's market hours before running (using IST time)
        if not is_market_hours(now_ist):
            logger.info(f"Outside market hours. Current time: {now_ist.strftime('%Y-%m-%d %H:%M:%S')} IST")
            return
        
        logger.info("=" * 60)
        logger.info(f"News Collector Cronjob triggered at {now_ist.strftime('%Y-%m-%d %H:%M:%S')} IST")
        logger.info("=" * 60)
        ...
        if success:
            logger.info("News Collector Cronjob completed successfully")
        else:
            logger.error("News Collector Cronjob completed with errors")
        
        # Update status file (use IST for MongoDB timestamps)
        status_data = {
        ...
        logger.error(f"News Collector Cronjob failed with error: {str(e)}")
        # Update status file with error (use IST for MongoDB timestamps)
        status_data = {
            "last_run": now_for_mongo().isoformat(),
            "last_status": "error"
        }
...
def main():
    ...
    logger.info("Starting NSE News Collector Scheduler")
