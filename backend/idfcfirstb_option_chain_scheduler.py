"""
Cronjob Scheduler for NSE IDFCFIRSTB Option Chain Data Collector
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('idfcfirstb_option_chain_scheduler.log', encoding='utf-8'),
...
STATUS_FILE = 'idfcfirstb_option_chain_scheduler_status.json'
...
def run_collector():
    """Execute the NSE IDFCFIRSTB option chain data collector"""
    collector = None
    try:
        now = datetime.now()
        ...
        logger.info("=" * 60)
        logger.info(f"IDFCFIRSTB Option Chain Cronjob triggered at {now}")
        logger.info("=" * 60)
        ...
        if success:
            logger.info("IDFCFIRSTB Option Chain Cronjob completed successfully")
        else:
            logger.error("IDFCFIRSTB Option Chain Cronjob completed with errors")
        ...
        logger.error(f"IDFCFIRSTB Option Chain Cronjob failed with error: {str(e)}")
...
def main():
    ...
    logger.info("Starting NSE IDFCFIRSTB Option Chain Data Collector Scheduler")
